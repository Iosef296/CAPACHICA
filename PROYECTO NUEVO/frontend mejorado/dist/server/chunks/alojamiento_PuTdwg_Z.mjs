...
import 'piccolore';
import { o as renderComponent, h as renderTemplate, q as Fragment, g as addAttribute, m as maybeRenderHead } from './server_80AW_iNR.mjs';
...
S/ ${o.precio} </span> <span style="font-size:0.75rem;color:var(--text3);margin-left:4px;">/ ${o.unidad}</span> </div> <button${addAttribute(`
                ...
                `, "style")}>Consultar →</button> </div> </div> </div>`)} </div> </div>  <div style="border-top:1px solid rgba(45,212,191,0.14);padding:4rem 1.5rem;"> <div style="max-width:900px;margin:0 auto;"> <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(200px,1fr));gap:1.5rem;"> ${[
  ...
  ].map((item) => renderTemplate`<div class="reveal" style="text-align:center;padding:1.5rem 1rem;"> <div style="font-size:2rem;margin-bottom:0.75rem;">${item.icon}</div> <div style="font-weight:700;color:var(--text);margin-bottom:0.4rem;font-size:0.95rem;">${item.title}</div> <div style="font-size:0.82rem;color:var(--text3);line-height:1.6;">${item.desc}</div> </div>`)} </div> </div> </div> `, "hero": ($$result2) => renderTemplate`${renderComponent($$result2, "Fragment", Fragment, { "slot": "hero" }, { "default": ($$result3) => renderTemplate` <span class="section-label">🏡 Hospedaje Auténtico</span> <h1 class="section-title" style="font-size:clamp(2.2rem,5.5vw,3.8rem);margin-bottom:1rem;">
Duerme en <span class="gradient-text">Capachica</span> </h1> <p style="font-size:1rem;color:var(--text2);line-height:1.78;max-width:520px;margin:0 auto 2rem;">
...
</p> <div style="display:flex;justify-content:center;gap:0.6rem;flex-wrap:wrap;"> ${["Todos", "Familia Anfitriona", "Lodge", "Hostal", "Casa Rural"].map((f, i) => renderTemplate`<span${addAttribute(`padding:0.45rem 1.1rem;border-radius:999px;font-size:0.82rem;font-weight:500;cursor:default;${i === 0 ? "background:linear-gradient(135deg,#0ea5e9,#0369a1);color:#fff;box-shadow:0 2px 12px rgba(56,189,248,0.3);" : "background:rgba(56,189,248,0.07);border:1px solid rgba(56,189,248,0.2);color:var(--text2);"}`, "style")}>${f}</span>`)} </div> ` })}` })}`;
