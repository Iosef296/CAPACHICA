...
const $$Destinos = createComponent(($$result, $$props, $$slots) => {
  ...
  return renderTemplate`${renderComponent($$result, "Layout", $$Layout, { "title": "Destinos — Capachica Turismo", "description": "Explora las comunidades ancestrales, paisajes andinos y la magia del Lago Titicaca desde la península de Capachica." }, { "default": ($$result2) => renderTemplate` ${maybeRenderHead()}<div style="min-height:100vh; background:var(--bg-main); padding-top:66px;"> <!-- Hero --> <div style="position:relative; padding:6rem 1.5rem 4rem; text-align:center; overflow:hidden;"> <div style="position:absolute;inset:0;background:radial-gradient(ellipse 70% 60% at 50% 0%,rgba(14,165,233,0.12) 0%,transparent 70%);pointer-events:none;"></div> <div style="position:absolute;top:20%;left:10%;width:300px;height:300px;background:radial-gradient(circle,rgba(56,189,248,0.05) 0%,transparent 70%);pointer-events:none;border-radius:50%;"></div> <div style="position:relative;z-index:1;max-width:700px;margin:0 auto;"> <span class="section-label reveal">🗺️ Explora la Península</span> <h1 class="section-title reveal" style="font-size:clamp(2.5rem,6vw,4rem);margin-bottom:1.2rem;">
Destinos <span class="gradient-text">Imperdibles</span> </h1> <p class="reveal" style="font-size:1.05rem;color:var(--text2);line-height:1.75;max-width:560px;margin:0 auto 2.5rem;">
...
</p> <!-- Stats rápidos --> <div class="reveal" style="display:flex;justify-content:center;gap:2.5rem;flex-wrap:wrap;"> ${[["8+", "Comunidades"], ["3820", "msnm"], ["12", "Islas"], ["500+", "años historia"]].map(([v, l]) => renderTemplate`<div style="text-align:center;"> <div style="font-family:var(--font-display);font-size:1.8rem;font-weight:800;color:var(--accent);line-height:1;">${v}</div> <div style="font-size:0.73rem;color:var(--text3);letter-spacing:0.1em;text-transform:uppercase;">${l}</div> </div>`)} </div> </div> </div> <!-- Destinos grid --> <div style="max-width:1200px;margin:0 auto;padding:0 1.5rem 6rem;"> <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(340px,1fr));gap:1.5rem;"> ${destinos.map((d, i) => renderTemplate`<div class="reveal card"${addAttribute(`padding:0;overflow:hidden;transition-delay:${i * 0.08}s;border-color:rgba(255,255,255,0.06);`, "style")}> <!-- Card header --> <div${addAttribute(`
              height:180px;
